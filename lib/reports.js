  ...
  return fs
    ...
    .map((f) => {
      try {
        ...
        return { chain: r.chain, token: r.token, name: r.meta?.name, symbol: r.meta?.symbol, score: r.score, grade: r.grade, clusters: r.metrics?.clusters, scannedAt: r.scannedAt, demo: !!r.meta?.demo };
      } catch {
        return null;
      }
    })
