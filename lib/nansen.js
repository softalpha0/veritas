...
import { CACHE_DIR, CALL_LOG, BUNDLED_CALL_LOG } from "./paths.js";
...
export function callStats() {
  // Locally both paths are the same file; on Vercel the committed log is added.
  const files = [...new Set([BUNDLED_CALL_LOG, CALL_LOG])].filter((f) => fs.existsSync(f));
